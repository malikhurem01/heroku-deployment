...
public class BidServiceImpl implements BidService {
    ...
    public BidDTO getBidHistory(Long productId) throws ProductDoesNotExistException {
        ...
        BidDTO bidResponse;
        if (bidsList.size() > 0) {
            latestBidderId = bidsList.get(bidsList.size() - 1).getUserId().getUserId();
            double max = bidsList.stream().mapToDouble(v -> v.getBidPrice()).max().orElseThrow(NoSuchElementException::new);
            bidResponse = new BidDTO(latestBidderId, (float) max, bidsList.size());
        } else {
            bidResponse = new BidDTO(latestBidderId, product.getStartPrice(), 0);
        }
        return bidResponse;
    }
